...
| `Address` | `street`, `city`, `zip` | A physical address |
...
| `HAS_ADDRESS` | Account → Address | `type` (billing/shipping) | Account associated with address |
...
│
├── queries/
│   ├── 01_basic_traversal.cypher   # Single-hop: who shares a phone?
│   ├── 02_shared_identifiers.cypher# Multi-identifier overlap
│   ├── 03_ring_detection.cypher    # N-hop fraud ring traversal
│   ├── 04_velocity_checks.cypher   # High-frequency transactions in a time window
│   └── 05_risk_scoring.cypher      # Composite risk score per account
│
├── system_design/
│   ├── ADR.md                      # Graph vs relational — decision record
│   ├── schema.md                   # Every node/edge modeling decision explained
...
| Neo4j Java Driver | 5.x | Connecting Java to Neo4j |
...
- 100 legitimate accounts in normal clusters
...
```bash
docker exec -it ringnet-neo4j cypher-shell -u neo4j -p password123 \
  --file /var/lib/neo4j/import/queries/03_ring_detection.cypher
```

Work through queries in numbered order — each builds on the previous.
...
**`schema.md`** — Detailed explanation of every node and edge modeling decision. Why Phone is a node instead of an Account property. Why Transaction is a node instead of purely an edge. Why relationships are directed.
...
Step 6   queries/01 through 05              → All queries return expected results
...
The documents in `system_design/` are structured as interview artifacts — ADR, schema rationale, and SQL vs Cypher comparison — not just notes.
